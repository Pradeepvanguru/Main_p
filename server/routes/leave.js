...
const handleEmailTimeout = async (leaveRequest, priority, nextPriorityHandler) => {
  ...
  }, 30 * 1000); // 1-minute delay for each priority stage (60 * 60 * 1000) + (30 * 60 * 1000)
};
...
const handleLeaveRequest = async (leaveRequest, priority, previousEmails = []) => {
  ...
  const emailMessage = `
  <div style="display:flex; align-items:center; justify-content:center; background-color:#4a4a4a; color:white; border:2px solid yellow; border-radius:1.5rem; max-width:600px; margin:0 auto;">
    <div style="padding:20px; margin:15px; width:100%;">
      <h2 style="text-align:center; font-size:24px; color:yellow;">Class Adjustment Request from <b style="color:white;">${leaveRequest.name}..!</b></h2>
      
      <table style="width:100%; margin-top:20px; font-size:18px; color:white; border-collapse:collapse;">
        <tbody>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Branch</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.branch}</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Year</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.year} year</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Semester</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.sem} sem</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Section</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.section}</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Subject Name</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.subj}</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Which Period</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.period}</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Period Timings</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${leaveRequest.startTime}</b> To <b>${leaveRequest.endTime}</b></td>
          </tr>
          <tr>
            <td style="padding:8px; border-bottom:1px solid white;">Leave Date</td>
            <td style="padding:8px; border-bottom:1px solid white;"><b>: ${formatDate(leaveRequest.leaveDate)}</b> on <b>${leaveRequest.dayName}</b></td>
          </tr>
        </tbody>
      </table>
      
      <div style="text-align:center; margin-top:20px;">
        <a href="http://localhost:5000/api/leave/accept/${leaveRequest._id}?acceptingEmail={{userEmail}}&priority=${priority}" style="display:inline-block; padding: 12px 24px; background-color: green; color: white; font-size:18px; text-decoration:none; margin:10px; border-radius:5px;">Accept</a>
        <a href="http://localhost:5000/api/leave/reject/${leaveRequest._id}?rejectingEmail={{userEmail}}" style="display:inline-block; padding: 12px 24px; background-color: red; color: white; font-size:18px; text-decoration:none; margin:10px; border-radius:5px;">Reject</a>
      </div>
    </div>
  </div>
`;
      ...
      handleEmailTimeout(leaveRequest, 2, async () => {
        ...
        handleEmailTimeout(leaveRequest, 3, async () => {
          ...
          handleEmailTimeout(leaveRequest, 4, async () => {
            ...
            setTimeout(async () => {
              ...
              if (leaveStatus && leaveStatus.status === 'Pending') {
                ...
                await sendEmail(
                  ...
                  'No Response Message',
                  `
                  <div style="background-color:#4a4a4a; color:white; padding:20px; border-radius:2rem; text-align:center; max-width:800px; margin:0 auto;">
                    <h1 style="font-size:28px; color:yellow; margin-bottom:20px;">No one accepted your leave request...</h1>
                    <p style="font-size:20px; margin-bottom:15px;">Try another day. Good luck!</p>
                    <h2 style="font-size:22px; color:white;">
                      Leave Date: <b>${formatDate(leaveRequest.leaveDate)}</b> on <b>${leaveRequest.dayName}</b>
                    </h2>
                  </div>
                  `
                );
            ...
            },  ( 30 * 1000)); // 2 minutes timeout 1:30 hours
            console.log('Time up ..No one accepted your request...try another day..');
            console.log("1.The pending  requests  will be deleted at 11:59 PM daily.");
            console.log("2.The Accepted records are deleted  after 3 days at midnight daily.");
          });
        });
      });
    });
...
router.get('/accept/:id', async (req, res) => {
  ...
  try {
    ...
    await sendEmail(
      ...
      `
      <div style="background-color:#4a4a4a; border:2px solid yellow; border-radius:3rem; display:flex; align-items:center; justify-content:center; color:white; padding:20px; max-width:600px; margin:0 auto;">
        <div style="padding:20px; text-align:center;">
          <h1 style="color:yellow; font-size:28px; margin-bottom:20px;">Your request has been accepted by <b style="color:white;">${acceptingUser.name}</b> 👋</h1>
          <h3 style="color:yellow; font-size:22px; margin-bottom:10px;">Faculty details:</h3>
          
          <ul style="list-style:none; padding:0; font-size:18px; color:white; text-align:left;">
            <li style="padding:5px 0;"><b>Branch:</b> ${acceptingUser.branch}</li>
            <li style="padding:5px 0;"><b>Year:</b> ${acceptingUser.year}</li>
            <li style="padding:5px 0;"><b>Semester:</b> ${acceptingUser.sem}</li>
            <li style="padding:5px 0;"><b>Section:</b> ${acceptingUser.section}</li>
            <li style="padding:5px 0;"><b>Subject:</b> ${acceptingUser.subj}</li>
            <li style="padding:5px 0;"><b>Leave Date:</b> ${formatDate(leaveRequest.leaveDate)} on <b>${leaveRequest.dayName}</b></li>
          </ul>
        </div>
      </div>
      `
