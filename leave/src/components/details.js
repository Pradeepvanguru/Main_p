...
function Table() {
    ...
    try {
      ...
      updatedRequests.forEach(request => {
        ...
        if (savedTimer && parseInt(savedTimer, 10) <= 0) {
          ...
          setTimers(prevTimers => ({
            ...prevTimers,
            [request._id]: 2 *60, // 8 hours in seconds 8*3600  || 2 * 60 * 1000 2 min
          }));
        }
      });
    } catch (error) {
  ...
  return (
    <div className='container py-4'>
      ...
      <div>
        {loading ? (
          <center><div className="text-center col-sm-2 p-2 rounded text-dark shadow-lg  fs-4">Loading....wait...</div></center>
        ) : (
            ...
            <div style={{ position: 'relative', height: '400px', overflowY: acceptedRequests.length > 5 ? 'scroll' : 'unset' }}>
              <table className="table table-hover table-striped text-center border-warning " style={{ tableLayout: 'fixed', width: '100%' }}>
                <thead className='table-primary ' style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                  <tr >
                    <th className='p-3 text-center' >Serial No.</th>
                    <th> Applyer Name</th>
                    <th> Leave Date</th>
                    <th className='col-sm-1'>Branch</th>
                    <th >Year</th>
                    <th >Semester</th>
                    <th className='col-sm-2'> Accepted Faculty Name</th>
                    <th className='col-sm-2'> Accepted Faculty Subject</th>
                    <th>Status</th>
                    <th>Timer</th>
                  </tr>
                </thead>
